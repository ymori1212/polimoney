...
class TestMetadataManager(unittest.TestCase):
    ...
    def test_update_file_status_success(self):
        """update_file_statusメソッドが成功ステータスを更新する場合のテスト"""
        metadata = FileMetadata(
            filename="test.pdf",
            original_url="https://example.com/test.pdf",
            organization="テスト団体",
        ...
        with patch('datetime.datetime') as mock_datetime:
            ...
            updated = self.manager.update_file_status(
                metadata=metadata,
                status="success",
                file_size=1000
            )
        
        self.assertEqual(updated.download_status, "success")
        self.assertEqual(updated.file_size, 1000)
        self.assertIsNotNone(updated.download_date)

    def test_update_file_status_failed(self):
        """update_file_statusメソッドが失敗ステータスを更新する場合のテスト"""
        metadata = FileMetadata(
            filename="test.pdf",
            original_url="https://example.com/test.pdf",
            organization="テスト団体",
        ...
        self.assertEqual(updated.download_status, "failed")
        self.assertEqual(updated.error, "テストエラー")
