...
class SeijishikinDownloader:
    ...
    def __init__(self, args: Namespace) -> None:
        ...
        self.name_filter: Optional[NameFilter] = NameFilter(args.name, args.exact_match) if args.name else None
        ...
        logger.debug("設定: 出力先=%s, 年度=%s, カテゴリ=%s, 名前フィルタ=%s, 待機時間=%s秒, 強制上書き=%s, ドライラン=%s, メタデータのみ=%s",
                    self.output_dir, self.years, self.categories, self.name_filter,
                    self.delay, self.force, self.dry_run, self.metadata_only)
    ...
    def download_all(self) -> bool:
        ...
        for report_url in report_urls:
            if isinstance(report_url, ReportListPageUrl):
                logger.info("報告書一覧 %s の処理を開始します: %s", report_url.year, report_url.url)
                self.process_report_list_page(report_url, report_url.year)
            else:
                logger.info("年度 %s の処理を開始します: %s", report_url.year, report_url.url)
    ...
    def process_report_list_page(self, report_list_url: ReportListPageUrl, year: str) -> None:
        """報告書一覧ページを処理
