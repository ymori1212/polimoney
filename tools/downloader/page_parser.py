...
class PageParser:
    ...
    def __init__(
        ...
        soup_factory: Callable[[str, str], BeautifulSoup] = None,
    ) -> None:
        """初期化
        ...
        self.soup_factory = soup_factory or (
            lambda text, parser: BeautifulSoup(text, parser)
        )
    ...
    def _extract_year_urls_from_soup(
        self, soup: BeautifulSoup, base_url: str, seasonal_report_only: bool
    ) -> List[YearPageUrl]:
    ...
    def get_year_and_report_urls(self) -> List[Union[YearPageUrl, ReportListPageUrl]]:
        ...
        return self._extract_year_urls_from_soup(
            soup, BASE_URL, seasonal_report_only=True
        )
    ...
    def _find_category_header(
        self, soup: BeautifulSoup, section_text: str
    ) -> Optional[object]:
        """カテゴリ見出しを探す
        ...
        for header in soup.find_all(["h1", "h2", "h3", "h4", "h5", "h6"]):
            if section_text in header.get_text():
                logger.debug(
                    "カテゴリ見出しを見つけました(見出しタグ): %s", section_text
                )
                return header
    ...
    def parse_report_list_page(
        self, report_list_url: ReportListPageUrl
    ) -> List[PageLink]:
