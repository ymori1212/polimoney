...
class PDFDownloader:
    ...
    def check_existing_file(
        self, save_path: str, metadata: FileMetadata
    ...
    def download_pdf(
        self, pdf_url: str, save_path: str, metadata: FileMetadata
    ) -> FileMetadata:
        """PDFファイルをダウンロード
        ...
        if not create_directory(os.path.dirname(save_path)):
            logger.error(
                "ディレクトリの作成に失敗したためスキップします: %s", save_path
            )
        ...
        while retry_count < max_retries:
            try:
                ...
                if self.robots_checker and not self.robots_checker.can_fetch(pdf_url):
                    logger.warning(
                        "robots.txtによりアクセスが禁止されています: %s", pdf_url
                    )
                    metadata.download_status = "failed"
                ...
                retry_count += 1
                logger.warning(
                    "ダウンロード中にエラーが発生しました（リトライ %d/%d）: %s, エラー: %s",
                    retry_count,
                    max_retries,
                    pdf_url,
                    e,
